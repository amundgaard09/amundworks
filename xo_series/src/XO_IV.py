...
ACTIVATIONS: dict[
    str,
    tuple[
        Callable[[xp.ndarray], xp.ndarray], Callable[[xp.ndarray], xp.ndarray] | None
    ],
] = {
    ...
    "softmax": (
        unicogni.softmax,
        None,  # IMplement softmax derivative
...
class DenseLayer:
    def __init__(
        ...
        self.weights: xp.ndarray = xp.random.randn(n_inputs, n_outputs) * xp.sqrt(
            2.0 / n_inputs
        )  # He initialization

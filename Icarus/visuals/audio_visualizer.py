...
class AudioVisualizer(QtWidgets.QMainWindow):
    ...
    def setup_windows_loopback(self):
        ...
        try:
            ...
            default_output_device = sounddevice.default.device[1]  # Index 1 is default output
            ...
            for idx, device in enumerate(sounddevice.query_devices()):
                if device["hostapi"] == wasapi_idx and device["max_output_channels"] > 0:
                    if device["name"] == output_info["name"]:
                        device_id = idx
                        break
            ...
            native_rate = int(sounddevice.query_devices(device_id)["default_samplerate"])
            uniCLI.console_print("ICARUS", "blue", f"Visualizing Audio From: {sounddevice.query_devices(device_id)['name']}")
            uniCLI.console_print("ICARUS", "blue", f"Hardware Sample Rate: {native_rate}Hz")
            ...
            uniCLI.console_print("ICARUS", "red", f"Loopback initialization failed: {e}", "red")
            uniCLI.console_print("ICARUS", "blue", "Falling back to default system input device...", "yellow")

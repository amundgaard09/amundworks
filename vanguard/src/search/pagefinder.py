...
def insert_json(path_to_json: Path, content_dict: dict) -> bool:
    ...
    try:
        ...
        uniCLI.console_print(
            "JSON Writer", "white", f"Error writing to JSON file: {e}", "red"
        )
        return False
    uniCLI.console_print(
        "JSON Writer",
        "white",
        f"Successfully wrote to JSON file: {path_to_json}",
        "green",
    )
    return True
...
def main(page: WikipediaPage) -> None:
    ...
    if page.exists():
        ...
        uniCLI.console_print(
            "PAGEFINDER", "white", f"Found Wikipedia page: {title}", "green"
        )
    ...
    try:
        ...
        uniCLI.console_print(
            "PAGEFINDER",
            "white",
            "File already exists! Use the search engine instead",
            "red",
        )
    except Exception as e:

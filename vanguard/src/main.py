from search.pagefinder import get_page, get_wiki, parse
from search.pagefinder import main as pagefinder
...
if __name__ == "__main__":
    if not username or not usermail:
        uniCLI.console_print(
            "MAIN", "white", "Username and usermail must be set to use Vanguard!", "red"
        )
        exit(1)

    uniCLI.console_print("Vanguard", "white", "Starting Vanguard...", "green")
    uniCLI.console_print(
        "Vanguard", "white", f"Logged in as: {username} ({usermail})", "green"
    )

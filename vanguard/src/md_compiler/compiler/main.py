...
def render_sections(sections: list[WikipediaPageSection], level: int = 2) -> str:
    uniCLI.console_print(
        "RENDERER", "blue", "Rendering sections into Markdown...", "green"
    )
    markdown = ""
    for sec in sections:
...
def preprocess_to_page_ir(page: WikipediaPage) -> PageIR:
    ...
    uniCLI.console_print(
        "IR PREPROCESSOR", "white", "Preprocessing page to PageIR...", "green"
    )
...
def extract_headings(ir: PageIR) -> PageIR:
    ...
    uniCLI.console_print(
        "HEADING EXTRACTOR", "white", "Extracting headings...", "green"
    )
...
def format_page(ir: PageIR) -> str:
    ...
    uniCLI.console_print(
        "MD FORMATTER", "white", "Formatting page into Markdown...", "green"
    )
    md = f"# {ir.title}\n\n"
...
def inject_wikilinks(IR: PageIR) -> str:
    ...
    uniCLI.console_print(
        "LINK INJECTOR",
        "white",
        "Injecting wikilinks into Markdown document...",
        "green",
    )
...
def split_into_sections(IR: PageIR) -> str:
    ...
    uniCLI.console_print(
        "SECTION SPLITTER", "white", "Splitting text into sections...", "green"
    )
...
def save_markdown(IR: PageIR, name: str, path: Path) -> None:
    ...
    if full_path.exists():
        continue_ = uniCLI.console_confirm(
            "MD SAVER",
            "white",
            f"Warning: File '{full_path}' already exists and will be overwritten. Overwrite? (y/n)",
            "yellow",
        )
        if not continue_:
...
def main(name: str, page: WikipediaPage, path: Path) -> None:
    ...
    uniCLI.console_print(
        "MDC PIPELINE", "white", "MDC Pipeline initialized... ", "green"
    )

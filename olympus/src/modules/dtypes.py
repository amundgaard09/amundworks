...
class _serializable:
    ...
    def _serialize_value(value):
        ...
        if isinstance(value, dict):
            return {
                key: _serializable._serialize_value(item) for key, item in value.items()
            }
    ...
    def _deserialize_value(cls, value, annotation=None):
        if annotation is None:
            if isinstance(value, dict):
                return {
                    key: cls._deserialize_value(item) for key, item in value.items()
                }
        ...
        if origin is dict:
            ...
            return {
                key: cls._deserialize_value(item, value_annotation)
    ...
    def from_dict(cls, data):
        if not isinstance(data, dict):
            raise TypeError(
                f"Expected a dictionary for {cls.__name__}.from_dict, got {type(data).__name__}"
            )
        ...
        for param_name, parameter in signature.parameters.items():
            ...
            if (
                param_name == "days"
                and param_name not in data
                and isinstance(data, dict)
                and data
            ):
                kwargs[param_name] = cls._deserialize_value(
                    data, annotations.get(param_name)
                )
            ...
            if param_name == "_duration" and "duration" in data:
                kwargs[param_name] = cls._deserialize_value(
                    data["duration"], annotations.get(param_name)
                )
            ...
            if param_name in {"city", "state", "country"} and "location" in data:
                ...
                if isinstance(location, list) and len(location) >= 3:
                    ...
                    kwargs[param_name] = cls._deserialize_value(
                        normalized[param_name], annotations.get(param_name)
                    )
            ...
            if param_name in data:
                kwargs[param_name] = cls._deserialize_value(
                    data[param_name], annotations.get(param_name)
                )
                consumed.add(param_name)
                continue
            ...
            if parameter.default is inspect._empty:
                raise KeyError(
                    f"Missing required field '{param_name}' when deserializing {cls.__name__}"
                )
...
class PRTable(_serializable):
    ...
    def __str__(self) -> str:
        pr_summary = ", ".join(
            f"{dist}: {time:.2f}s" for dist, time in self.pr_data.items()
        )
...
class Step(_serializable):
    ...
    def __str__(self) -> str:
        return f"{self.name} | {self.sport} | {self.type} - {self.desc} for {self._duration // 3600} hours and {self._duration % 60} minutes at {color_text(self.thrz, _color_from_zone(self.thrz))} bpm (TRPE: {color_dtypes.color_text(self.trpe, _color_from_trpe(self.trpe))})"
    ...
    def __eq__(self, other) -> bool:
        ...
        return (
            ...
            and self.thrz == other.thrz
            and self.trpe == other.trpe
        )
...
class Exercise(_serializable):
    ...
    def __init__(self, name: str, desc: str | None, type: str, steps: list[Step]):
        self.name = name
        self.desc = desc
        self.type = type
...
class LoggedSession(Session):
    ...
    def __init__(
        ...
        super().__init__(
            name=name, desc=desc, type=type, exercises=exercises, start_at=started_at
        )
...
class WeekPlan(_serializable):
    ...
    def duration(self):
        return sum(
            session.duration for sessions in self.days.values() for session in sessions
        )
...
class TrainingBlock(_serializable):
    ...
    @property
    def duration(self) -> float:
        return sum(week.duration for week in self.weeks)
...
class Event(_serializable):
    def __init__(
        ...
        priority: Literal["Primary", "Secondary", "Tertiary", "Supporting"]
        | None = None,
        notes: str | None = None,
    ):
        self.name = name
...
class AthleteProfile(_serializable):
    def __init__(
        ...
        level: Literal["Beginner", "Intermediate", "Advanced", "Elite", "Professional"]
        | None = None,
    ):
        self.name = name
        self.age = age

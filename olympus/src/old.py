...
def evening_log():
        ...
        with open(LOGPATH, "r") as f:
            ...
            if lines:
                for line in lines:
                    if line.startswith(
                        f"E|{date_str[:10]}"
                    ):  # Check for today's date in the log
                        ...
                        print(
                            f"Step Goal Met: {'Yes' if log_details[5] == 'True' else 'No'}"
                        )
                        print(
                            f"Injury/Discomfort: {'Yes' if log_details[6] == 'True' else 'No'}"
                        )
    ...
    daily_completion_score = (
        len(completed_workouts) / len(SCHEDULE[datetime_now.strftime("%A").lower()])
    ) * 100
    print(
        color_text(
            f"Daily Completion Score: {daily_completion_score:.2f}%",
            "green"
            if daily_completion_score >= 85
            else "yellow"
            if daily_completion_score >= 50
            else "red",
        )
    )

    stepgoalbool = questionary.confirm("Did you meet your step goal today?").ask()
    injurybool = questionary.confirm(
        "Did you experience any injuries or discomfort today?"
    ).ask()
    sleepquality = questionary.select(
        "How was your sleep quality?", choices=["Excellent", "Good", "Fair", "Poor"]
    ).ask()
    nutriscale: int = questionary.select(
        "Rate your nutrition today:", choices=[str(i) for i in range(1, 11)]
    ).ask()
    mentalscale: int = questionary.select(
        "Rate your mental health:", choices=[str(i) for i in range(1, 11)]
    ).ask()
    ...
    print(
        color_text(
            f"Overall Day Score: {dayscore}/100",
            "green" if dayscore >= 85 else "yellow" if dayscore >= 50 else "red",
        )
    )
...
def scheduleoverview():
    ...
    if schedule is None:
        ...
        for day, workouts in schedule.items():
            print(
                color_text(
                    f"{day.capitalize()}: {', '.join(workouts) if workouts else 'No workouts scheduled'}",
                    "green",
                )
            )
...
def exercisebuilder():
    ...
    while addsteps:
        ...
        print(
            color_text(f"Added steps: {[step.name for step in steps]}", "green") + "\n"
        )

        if (
            not firststepadded
        ):  # Prevents copy option on first step before any steps exist
            copystep = False
        else:
        ...
        if copystep:
            copystepname = questionary.select(
                "Select a step to copy:", choices=[step.name for step in steps]
            ).ask()
        else:
            copystepname = None
        ...
        if copystepname:
            ...
            if selected_step:
                ...
                print(
                    color_text("Selected step not found. Creating new step.", "yellow")
                )
        if stepadded:
    ...
    newexercise = Exercise(name=exercisename, steps=steps)
...
def sessionbuilder():
    ...
    seshtype = questionary.select(
        "Session Type:",
        choices=[GOALS[idx].capitalize() for idx, _ in enumerate(GOALS)],
    ).ask()
    duration: int = 0
...
def sessionviewer():
    ...
    if not sessions:
        action = questionary.confirm(
            color_text("No sessions found. Build a new one?", "yellow")
        ).ask()
        if action:
            sessionbuilder()
        else:
...
def add_event():
    ...
    priority: str = questionary.select(
        "Priority", choices=["Primary", "Secondary", "Tertiary", "Supporting"]
    ).ask()
...
def racecalendar():
    ...
    if not events:
        print(
            color_text(
                "No upcoming events found. Add an event to get started!", "yellow"
            )
            + "\n"
        )
    else:
        for event in events:
            print("------------------------------")
            print(
                f"{event.name} - {event.date} - {event.type} - Priority:"
                + color_text(
                    " " + event.priority,
                    "red"
            ...
            if isinstance(event.distance, list):
                print(
                    color_text(
                        f">>> Distances (km): Swim: {event.distance[0]}, Bike: {event.distance[1]}, Run: {event.distance[2]}",
                        "yellow",
                    )
                )
            else:
            ...
            print(
                color_text(
                    f">>> Location: {', '.join(event.location) if event.location else 'N/A'}",
                    "yellow",
                )
            )
            if event.notes:
...
def pr_tracker():
    ...
    choice = questionary.select(
        "Select an option:", choices=["View PRs", "Add PR", "Return"]
    ).ask()
    match choice:
        case "View PRs":
            clear_terminal()

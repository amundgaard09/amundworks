...
const { commandExists, getOpencodeDir, getClaudeDir, readFile, writeFile, log, runCommand } = require('./utils');
 ...
 *
 * Detection priority:
 * 1. Environment variable OPENCODE_PACKAGE_MANAGER (fallback: CLAUDE_PACKAGE_MANAGER)
...
function getPackageManager(options = {}) {
  ...
  // 1. Check environment variable (OPENCODE preferred, CLAUDE for backward compat)
  ...
  // 2. Check project-specific config (.opencode preferred, .claude for backward compat)
  const projectConfigPath = path.join(projectDir, '.opencode', 'package-manager.json');
  const legacyProjectConfigPath = path.join(projectDir, '.claude', 'package-manager.json');
  let projectConfig = readFile(projectConfigPath);
  if (!projectConfig) {
    projectConfig = readFile(legacyProjectConfigPath);
  }
  if (projectConfig) {
    try {

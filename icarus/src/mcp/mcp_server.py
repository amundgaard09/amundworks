...
from .types import InputSchema, MCPProperty, MCPTool
...
class MCPServer:
    ...
    def load_executable(skill_path: Path) -> ModuleType:
        ...
        modspec = importlib.util.spec_from_file_location("skill_module", py_path)
    ...
    def build_input_schema(
        self, schema_dict: dict[str, dict] | None
    ) -> InputSchema | None:
        """
        ...
        for name, meta in schema_dict.items():
            properties.append(
                MCPProperty(
                    name=name, dtype=meta["type"], required=meta.get("required", True)
                )
            )
    ...
    def load_tool(self, tool_name: str) -> MCPTool:
        ...
        try:
            ...
            uniCLI.console_print(
                "MCP SERVER", "green", f"ERROR: {tool_path.name} config file not found."
            )

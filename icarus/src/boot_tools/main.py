...
from durapy.durapy.uniCLI import Console, console_print
...
def check_windows_wifi() -> bool:
    ...
    try:
        ...
        console_print(
            "ICARUS INITIALIZER",
            "red",
            "Wi-Fi check failed: UnicodeDecodeError",
            "yellow",
        )
...
def check_linux_wifi() -> bool:
    ...
    try:
        output = (
            subprocess.check_output("nmcli -t -f WIFI g", shell=True).decode().strip()
        )
...
def check_wifi(console: Console) -> None:
    ...
    if not checkfunc():
        console.end_task(
            "Checking Wi-Fi",
            success=False,
            error_msg="No active Wi-Fi connection found.",
        )
        exit(-1)
...
def check_microphone(console: Console) -> None:
    ...
    try:
        ...
        console.end_task(
            "Checking Microphone",
            success=False,
            error_msg=f"Error checking microphone: {e}",
        )
        exit(-1)

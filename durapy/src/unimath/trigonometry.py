...
def cosine_rule(len_A: float, len_B: float, angle_A: float) -> float:
    return math.sqrt(len_A ** 2 + len_B ** 2 - ((2 * len_A * len_B) * math.cos(math.radians(angle_A))))
def reverse_cosine_rule(len_A: float, len_B: float, len_C: float) -> tuple[float, float, float]:
    ...
    return (
        math.degrees(math.acos((len_B ** 2 + len_C ** 2 - len_A ** 2) / (2 * len_B * len_C))),  # AngleA
        math.degrees(math.acos((len_C ** 2 + len_A ** 2 - len_B ** 2) / (2 * len_C * len_A))),  # AngleB
        math.degrees(math.acos((len_A ** 2 + len_B ** 2 - len_C ** 2) / (2 * len_A * len_B)))   # AngleC
    )

...
class Dimension:
    ...
    def __mul__(self, other: Dimension) -> Dimension:
        return Dimension(tuple(Fraction(a + b) for a, b in zip(self.exponents, other.exponents)))
    ...
    def __rtruediv__(self, other: Dimension) -> Dimension:
        return Dimension(tuple(Fraction(b - a) for a, b in zip(self.exponents, other.exponents)))

    def __pow__(self, power: int | float | Fraction) -> Dimension:
        return Dimension(tuple(Fraction(power) * x for x in self.exponents))
    def __rpow__(self, base: int | float) -> Dimension:
...
class Unit:
    ...
    def __rpow__(self, base: int | float) -> Unit:
        return Unit(
            symbol=self.symbol,
...
class Quantity:
    ...
    def __init__(self, value: float | complex | Quantity, unit: Unit) -> None:
    ...
    def __pow__(self, power: Quantity | float | int) -> Quantity:
        if not isinstance(power, (int, float)):
            raise TypeError("Power must be a scalar number.")
        new_dims = self.unit ** power
        return Quantity(self._value ** power, new_dims)
    def __rpow__(self, other: Quantity | float | int) -> Quantity:
        if isinstance(other, Quantity):
            return Quantity(other._value ** self._value, self.unit) # What about unit change?
        else:
            return Quantity(other ** self._value, self.unit)
    ...
    def __ge__(self, other: float | int | Quantity) -> bool:
        if isinstance(other, Quantity):
            if self.unit != other.unit:
                raise ValueError(f"Cannot compare units of different dimensions: {self.unit} vs {other.unit}")
            return self._value.real >= other._value.real
        return NotImplemented
    def __gt__(self, other: float | int | Quantity) -> bool:
        if isinstance(other, Quantity):
            if self.unit != other.unit:
                raise ValueError(f"Cannot compare units of different dimensions: {self.unit} vs {other.unit}")
            return self._value.real > other._value.real
        return NotImplemented
    def __le__(self, other: float | int | Quantity) -> bool:
        if isinstance(other, Quantity):
            if self.unit != other.unit:
                raise ValueError(f"Cannot compare units of different dimensions: {self.unit} vs {other.unit}")
            return self._value.real <= other._value.real
        return NotImplemented
    def __lt__(self, other: float | int | Quantity) -> bool:
        if isinstance(other, Quantity):
            if self.unit != other.unit:
                raise ValueError(f"Cannot compare units of different dimensions: {self.unit} vs {other.unit}")
            return self._value.real < other._value.real

...
def mach_number(vel: float, mach: float = MACH.quantity._value.real) -> str:
    ...
    ratio = vel / mach
    label = ('SUBSONIC' if ratio < 1 else 'TRANSONIC' if abs(ratio - 1) < 0.01 else 'SUPERSONIC' if ratio < 5 else 'HYPERSONIC' if ratio < 10 else 'HIGH-HYPERSONIC')
    color = ('red'      if ratio < 1 else 'yellow'    if ratio == 1            else 'green'      if ratio < 5 else 'blue'       if ratio < 10 else 'violet')

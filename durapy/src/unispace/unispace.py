...
def orbital_period(semi_major_axis: float, M: float, m: float) -> Quantity:
    return Quantity((2 * math.pi * math.sqrt(semi_major_axis ** 3 / (G * (M + m)))), UNITS["S"])
def orbital_velocity(orbital_radius: float = EARTH_R, mass: float = EARTH_M) -> Quantity:
    return Quantity((math.sqrt((G * mass) / orbital_radius)), UNITS["m/s"])
...
def tsiolkovsky_rocket_equation(exhaust_velocity: float, initial_mass: float, final_mass: float) -> Quantity:
    return Quantity((exhaust_velocity * math.log(initial_mass / final_mass)), UNITS["Δv"])
